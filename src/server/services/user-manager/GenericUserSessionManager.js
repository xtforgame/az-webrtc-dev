...
export default class GenericUserSessionManager {
  ...
  loginWithToken(rcPeer, token) {
    ...
    if (session) {
      ...
      const wsSessionNumber = this.userSessionCounters[session.userid] = this.userSessionCounters[session.userid] ?
        this.userSessionCounters[session.userid] + 1 : 1;

      const wsSession = fakeUserManager.authenticateFromToken(token, { wsSessionNumber });

      return this.userSessionMgr.login(wsSession.userid, wsSession.token, {
        session,
        rcPeer,
      })
      .then(() => wsSession);
